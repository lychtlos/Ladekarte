...
let keinServer = false;           // rein statisch ausgeliefert (z. B. GitHub Pages)
let kontoTab = "login";
...
async function kontoPruefen(){
  try{
    ...
    if(e.status === 404 || e.status === 405){
      // Es antwortet zwar etwas, aber kein Ladekarte-Server: statische Auslieferung.
      keinServer = true;
      setzeSync("lokal", "Ohne Server ausgeliefert — Konten sind hier nicht möglich.");
    }else{
...
function renderKonto(){
  ...
  if(keinServer){
    $("kontoTitel").textContent = "Kein Konto möglich";
    body.innerHTML = `
      ${statusBlock}
      <p class="lead">Diese Fassung der App wird rein statisch ausgeliefert — es läuft kein Server dahinter,
        der Konten verwalten könnte. Alles andere funktioniert: Karte, Belegung und der Tarifvergleich.</p>
      <p class="lead">Deine Ladekarten liegen in diesem Browser. Sie bleiben erhalten, solange du die
        Browserdaten für diese Seite nicht löschst — auf ein anderes Gerät wandern sie so aber nicht.</p>
      <p class="fuss">Für die geräteübergreifende Anmeldung muss <code>server/server.js</code> laufen,
        also z. B. auf einem eigenen Rechner oder bei einem Hoster, der Node ausführt.
        Die Anleitung dazu steht in der README des Projekts.</p>`;
    return;
  }
